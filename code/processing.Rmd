...
chart_languages <- c('eu', 'es', 'ko', 'fi', 'et', 'kn')
...
for (file in csv_files) {
  ...
  predicted_x <- predict(theilsen_model, newdata = data.frame(y = frequency))
  predicted_x2 <- predict(theilsen_model_no_mb, newdata = data.frame(y = frequency))
  score1 <- abs(rank[1]-predicted_x[1])^2+abs(rank[length(predicted_x)]-predicted_x[length(predicted_x)])^2
  score2 <- abs(rank[1]-predicted_x2[1])^2+abs(rank[length(predicted_x)]-predicted_x2[length(predicted_x)])^2

  scores_mb_ts <- c(scores_mb_ts, sqrt(score1/2))
  scores_mb <- c(scores_mb, sqrt(score2/2))
  ...
  predicted_x <- predict(theilsen_model, newdata = data.frame(y = n))
  predicted_x2 <- predict(theilsen_model_no_mb, newdata = data.frame(y = n))
  score1 <- abs(f[1]-predicted_x[1])^2+abs(f[length(predicted_x)]-predicted_x[length(predicted_x)])^2
  score2 <- abs(f[1]-predicted_x2[1])^2+abs(f[length(predicted_x)]-predicted_x2[length(predicted_x)])^2

  scores_mb_ts2 <- c(scores_mb_ts2, sqrt(score1/2))
  scores_mb2 <- c(scores_mb2, sqrt(score2/2))
  ...
  scores_mb_ts3 <- c(scores_mb_ts3, sqrt(score1/2))
  scores_mb3 <- c(scores_mb3, sqrt(score2/2))
